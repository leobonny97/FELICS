...
class Contesto:
    ...
    def searchContesti_dec(self, high, low, code):
        delta = int(high - low)
        pos = -1
        aggiornare = False
        ...
        if pos != -1:  # è stato trovato il contesto cercato
            array_di_valori1_dec = self.arr_contesti_dec[pos + 1]  # recupero i valori
            ...
            # print("il contesto non è presente, lo aggiungo")
            Contesto.addContestoeValori_dec(self, high, low, code)
            k = 2
            return k, aggiornare

    def aggiornaContesto_dec(self, high, low, p):
        delta = int(high - low)
        pos = -1
        for idx, e in enumerate(self.arr_contesti_dec):
            if len(e) == 1:  # allora è un contesto
        ...
        if pos != -1:  # è stato trovato il contesto cercato
            array_di_valori1_dec = self.arr_contesti_dec[pos + 1]  # prendo i valori i valori
            for k in range(4):
                if p < low:
                    length = len(GolombRice.golomb_rice(low - p - 1, k))
            ...
            self.arr_contesti_dec[pos + 1] = array_di_valori1_dec

    def addContestoeValori_dec(self, high, low, p):
        arr_valori_dec = []
        delta = int(high-low)
        for k in range(4):
            if p < low:
